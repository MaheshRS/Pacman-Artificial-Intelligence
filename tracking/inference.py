...
class ExactInference(InferenceModule):
        ...
        if noisyDistance is None:
            ...
            for p in self.beliefs:
                ...
                if emissionModel[true_distance] > 0:
                    all_possible[p] = self.beliefs[p] * emissionModel[true_distance];
                else:
                    all_possible[p] = 0.0
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # print self.ghostAgent.state()
        # position = gameState.getGhostPosition(self.ghostAgent.index)
        # # for ghost_position in ghost_positions:
        # #     self.setGhostPosition(gameState, ghost_position)
        # #     position_distribution = self.getPositionDistribution(gameState)
        # print position
        # exit()

        possible_beliefs = util.Counter()
        for p in self.legalPositions:
            possible_beliefs[p] = 0.0

        for ghost_position in self.legalPositions:
            updated_game_state = self.setGhostPosition(gameState, ghost_position)
            position_distribution = self.getPositionDistribution(updated_game_state)

            for p in position_distribution:
                possible_beliefs[p] += (self.beliefs[ghost_position] * position_distribution[p])
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # print "#Particles: ", self.numParticles
        # print "#Legal postions: ", len(self.legalPositions)
        # print "#Particles per location: ", self.num_particles_per_location

        # Uniformly distribute the particles across the location until the total number of particles equals 'self.numParticles'.
        count = 0
        while count < self.numParticles:
            for position in self.legalPositions:
                ...
                self.particle_distribution.append(position)
                count += 1
    ...
    def observe(self, observation, gameState):
        ...
        # For reference.
        '''The emissionModel below stores the probability of the noisyDistance for
        any true distance you supply. That is, it stores P(noisyDistance | TrueDistance).'''

        weights = util.Counter()

        # Get weights for the particles.
        if noisyDistance is None:
            weights[self.getJailPosition()] = self.numParticles * 1.0
        else:
            for p in self.particle_distribution:
                true_distance = util.manhattanDistance(p, pacmanPosition)
                weights[p] += emissionModel[true_distance]

        weights.normalize()

        # Check if all the weights of the belief is 0.
        if weights.totalCount() == 0:
            # All particles have weight 0. This basically restarts the whole particle
            # print "Restarting..."
            self.initializeUniformly(gameState)
        else:
            # Resample the particle list
            # Update the particle distribution.
            self.particle_distribution = None
            self.particle_distribution = []

            for i in range(0, self.numParticles):
                self.particle_distribution.append(util.sample(weights))
    ...
    def elapseTime(self, gameState):
        ...
        # Resample particles and normalize it.
        resampled_particles = []
        for p in self.particle_distribution:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            resampled_particles.append(util.sample(newPosDist))

        self.particle_distribution = None
        self.particle_distribution = resampled_particles
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        position_counter = util.Counter();
        for p in self.particle_distribution:
            position_counter[p] += 1;

        position_counter.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        legal_positions_permutations = list(itertools.permutations(self.legalPositions, self.numGhosts))
        random.shuffle(legal_positions_permutations)

        count = 0
        while count < self.numParticles:
            for position in legal_positions_permutations:
    ...
    def observeState(self, gameState):
        ...
        # All ghosts are captured.
        if noisyDistances is None:
            # Keep track of the jail positions of all the ghosts.
            all_ghost_captured_position = []
            for i in range(self.numGhosts):
                all_ghost_captured_position.append(self.getJailPosition(i))

            # Update the weights of the jail positon of the captured ghosts.
            weights[tuple(all_ghost_captured_position)] = 1 * self.numParticles
        else:
            # The ghosts are not captured. Update the particle distribution for the ghost positions.
            for particle in self.particles:
                value = 1
                for idx in range(0, self.numGhosts):
                    if noisyDistances[idx] is None:
                        particle = self.getParticleWithGhostInJail(particle, idx)
                    else:
                        true_distance = util.manhattanDistance(pacmanPosition, particle[idx])
                        value *= emissionModels[idx][true_distance]

                weights[particle] += value

        # Normalize the weights.
        weights.normalize()

        # Check if the ghosts are captured.
        if weights.totalCount() == 0:
            self.initializeParticles()
        else:
            particle_list_l = []
            for i in range(0, self.numParticles):
                particle_list_l.append(util.sample(weights))

            self.particles = None
            self.particles = particle_list_l
