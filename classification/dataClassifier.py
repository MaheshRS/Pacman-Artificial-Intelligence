...
def enhancedFeatureExtractorDigit(datum):
    ...
    # 1. The gradient of the pixels in horizontal and vertical direction.
    # 2. Continious pixels in the vertical and horizontal direction.
    # 3. Continious pixels counts through out the digit image.

    "*** YOUR CODE HERE ***"
    # Compute gradients
    for x in range(1, DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            features[("h_left", x, y)] = int(datum.getPixel(x, y) > datum.getPixel(x - 1, y))
            features[("v_bottom", x, y)] = int(datum.getPixel(x, y) > datum.getPixel(x, y - 1))
            features[("horizontal_continious", x, y)] = int(datum.getPixel(x, y) == datum.getPixel(x - 1, y))
            features[("vertical_continious", x, y)] = int(datum.getPixel(x, y) == datum.getPixel(x, y - 1))    
            
    # Find all the regions that ae continious white.
    contigious = 0
    ...
    features["contigious_1"] = contigious % 2;
    features["contigious_2"] = (contigious/2) % 2;
    features["contigious_3"] = (contigious/4) % 2;

    ''''#for x in range(0, DIGIT_DATUM_WIDTH):
        #idx = 0
        #idy = 0
        #for y in range(0, DIGIT_DATUM_HEIGHT):
            #idx += int(datum.getPixel(x, y))
            #idy += int(datum.getPixel(y, x))
            #features[("val_x", x, y)] = idx + ((-1 + int(datum.getPixel(x, y))))
            #features[("val_y", y, x)] = idy + ((-1 + int(datum.getPixel(x, y))))
            
    #print features["active_pixels_center_left"]
    #print features["active_pixels_center_right"]
    #print features["active_pixels_center_top"]
    #print features["active_pixels_center_bottom"]
    #print features["aspect_ratio"]
    #print'''
...
def enhancedPacmanFeatures(state, action):
    ...
    # Get the food count.
    successor = state.generateSuccessor(0, action)
    food_count = successor.getFood().count()
    features["foodcount"] = food_count;

    # Get the nearest food distance.
    current_position = successor.getPacmanPosition()
    ghost_positions = successor.getGhostPositions()
    food_positions = state.getFood().asList()
    nearest_food_distance = min([util.manhattanDistance(current_position, position) for position in food_positions])
    features["nearestFoodDistance"] = nearest_food_distance

    # Get the nearest ghost distance.
    min_ghost_postion = None;
    for position in ghost_positions:
        distance = util.manhattanDistance(current_position, position)
        if min_ghost_postion == None:
            min_ghost_postion = distance
        elif distance < min_ghost_postion:
            min_ghost_postion = distance
    
    features["nearestGhostDistance"] = min_ghost_postion
