    ...
    def train( self, trainingData, trainingLabels, validationData, validationLabels ):
        ...
        for iteration in range(self.max_iterations):
            ...
            for i in range(len(trainingData)):
                ...
                # Get the guessed label.
                high_score_label = self.classify([feature_vector])[0]

                # Update the weights appropriately
                actual_label = trainingLabels[i];

                if actual_label != high_score_label:
                    self.weights += feature_vector[0][actual_label]
                    self.weights -= feature_vector[0][high_score_label];
